...
int equalStacks(int h1_count, int* h1, int h2_count, int* h2, int h3_count, int* h3) {
    int s1=0,s2=0,s3=0;
    for(int k=0;k<h1_count;k++) s1+=h1[k];
    for(int k=0;k<h2_count;k++) s2+=h2[k];
    for(int k=0;k<h3_count;k++) s3+=h3[k];
    
    int a=0,b=0,c=0;
    
    while(1){
        if(a==h1_count||b==h2_count||c==h3_count) return 0;
        if(s1==s2 && s2==s3) return s1;
        if(s1>=s2 && s1>=s3) s1-=h1[a++];
        else if(s2>=s1 && s2>=s3) s2-=h2[b++];
        else s3-=h3[c++];
    }
}
