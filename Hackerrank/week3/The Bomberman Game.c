...
char** detonate(int r,int c,char** grid){
    char** nG=malloc(r * sizeof(char*));
    for(int i=0;i<r;i++){
        nG[i]=malloc((c+1)*sizeof(char));
        for(int j=0;j<c;j++) nG[i][j]='O';
        nG[i][c]='\0';
    }
...
char** bomberMan(int n, int grid_count, char** grid, int* result_count) {
    *result_count=grid_count;
    ...
    if(n==1) return grid;
    if(n%2==0){
        char** f=malloc(grid_count*sizeof(char*));
        for(int i=0;i<grid_count;i++){
            f[i]=malloc((c+1)*sizeof(char));
            for(int j=0;j<c;j++) f[i][j]='O';
            f[i][c]='\0';
        }
        return f;
    }
    char** first=detonate(grid_count,  c, grid);
    if(n%4==3) return first;
    return detonate(grid_count,  c, first);;
